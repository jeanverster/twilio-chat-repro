...
const credentialsFromFile = require("../config.json");
...
const TokenProvider = {
  getToken: function (identity, pushChannel, customTTL) {
    ...
    if (
      !credentialsFromFile.tokenGenerator ||
      ...
      throw new Error(
        `${new Date()} [token-generator] [ERROR] No full credentials found in configuration.json`
      );
    }
    ...
    let grant = new AccessToken.ChatGrant({
      serviceSid: credentialsFromFile.tokenGenerator.serviceSid,
    });
    if (pushChannel && pushChannel !== "none") {
      if (credentialsFromFile.tokenGenerator[pushChannel]) {
        grant.pushCredentialSid =
          credentialsFromFile.tokenGenerator[pushChannel];
      }
    }
